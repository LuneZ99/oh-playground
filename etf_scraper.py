...
def scrape_etf_components(fund_id):
    ...
    """
    # Instead of scraping the HTML page directly, we'll try to find the API endpoint
    # that provides the component stocks data
    
    # First, let's set up headers to mimic a browser
    headers = {
    ...
    api_url = f"https://query.sse.com.cn/commonQuery.do?jsonCallBack=jsonpCallback&isPagination=false&sqlId=FUND_CONSTITUENT_STOCK_DETAIL_SEARCH&FUNDID={fund_id}&_={int(time.time() * 1000)}"
    ...
    try:
        ...
        json_text = re.search(r'jsonpCallback\((.*)\)', jsonp_text).group(1)
